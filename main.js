	...
	*/
	function news_ticker(headlines) {
		...
		for (var i=0; i < headlines.length; i++) {
			fullString += headlines[i] + ' ';
		}
		var i = 0;
		while (i < fullString.length) {
			stringResult += fullString[i];
			if (stringResult.length === 140) {
				break;
			}
			i++;
		}
...
/*
	...
	*/
	function front_x(words) {
		...
		// print(wordsArray);
		// resArray = wordsArray.concat(xArray);
		
		return xArray.concat(wordsArray);
	}
	print("front_x");
...
/*
	...
	*/
	...
	function linear_merge(list1, list2) {
		var resArray = list1.concat(list2);
		resArray.sort();
